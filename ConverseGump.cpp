...
ConverseGump::ConverseGump(Configuration *cfg, Font *f)
{
 ...
 GUI_Widget::Init(NULL, x+x_off, y+y_off, scroll_width * 8, 168);
 npc_portrait = NULL;
...
void ConverseGump::Display(bool full_redraw)
{
	MsgText token;
	 ...
	 for(iter=display_text.begin();iter != display_text.end() ; iter++)
	   {
	    token = *iter;
		token.font->drawString(screen, token.s.c_str(), area.x + PORTRAIT_WIDTH + 8 + total_length * 8, area.y, 0); //FIX for hardcoded font height
